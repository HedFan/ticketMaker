<template>
  ...
  <div v-else>
    <div v-if="!approvedTickets.length">Approuve a ticket from the Ticket Queue</div>
    <div
      v-for="(ticket, index) in approvedTickets"
      :key="ticket.id"
...
const approvedTickets = computed(() =>
  ticketsData.tickets.filter((item) => item.isApprouve === true),
)
</script>
