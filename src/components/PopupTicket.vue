<template>
  <div class="fixed inset-0 flex items-center justify-center bg-black opacity-60">
  </div>
  <div
    ...
    <div class="flex inline-flex mb-3">
      ...
      <div class="mr-4">
        <select
          ...
          <option v-for="option in PARTNER_OPTIONS"
      ...
      <div>
        ...
        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400"
        >
          <option v-for="option in STATUS_OPTIONS"
                  :key="option" :value="option"
          >{{ option }}</option>
    ...
    <div class="mb-3">
        ...
        >
          ...
          <div class="relative mb-1"
            ...
            <button
                ...
                class="absolute top-0 right-[-14px] text-red-500 hover:text-red-700 cursor-pointer">×</button>
          </div>
            <span
                @click="item.style.push('')"
                class="text-gray-500 cursor-pointer p-1 hover:text-blue-500">+</span>
          </div>
      ...
      <div>
        <button
            ...
            class="bg-transparent border-solid border text-gray-500 cursor-pointer p-2 hover:text-blue-500 rounded-lg">Add a new line</button>
      </div>
    ...
    <div class="flex justify-between">
      <button
        ...
        bg-gray-900 hover:bg-gray-700 disabled:bg-gray-400 disabled:cursor-not-allowed cursor-pointer"
      >
      ...
      <button
          @click="toggleApprove"
          class="px-4 py-2 rounded-lg text-white transition
        bg-red-800 hover:bg-gray-700 cursor-pointer"
      >
...
const openTicketData = computed(() => ticketData.tickets.find((item => item.id === usePopup.currentOpenTicketId)));
...
const toggleApprove = async () => {
  ...
  await ticketData.setUpdatedData(currentId,
      { isApprouve: !currentValue, status: toggleStatus }, true);
}

const sentToPartner = async () => {
  const currentId = openTicketData.value?.id as number;
  await ticketData.setUpdatedData(currentId,
      { isSentToPartner: true, status: "Sent" }, true);
